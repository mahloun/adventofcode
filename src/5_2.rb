...
#part2
def contains_two_not_overlapping_pair() contains %r{(\w{2}).*\1} end
def contains_bridge() contains %r{(\w).\1} end

def is_nice? string
  # has_three_vowels.call(string) \
  #   && contains_twice_in_row.call(string) \
  #   && excludes_forbidden_strings.call(string)
  contains_bridge.call(string) && contains_two_not_overlapping_pair.call(string)
end
